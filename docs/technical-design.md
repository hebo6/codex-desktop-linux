...
| 凭据 | Linux Secret Service | 保存令牌和敏感认证字段 |
| 日志 | tracing | 结构化日志和脱敏 |
...
flowchart LR
    ...
    SSH --> Remote
    Bridge --> Files[远程文件服务]
    Bridge --> Secrets[Linux Secret Service]
    Bridge --> DB[SQLite]
...
令牌、代理密码、SSH 密码、密钥口令和敏感环境变量存入 Linux Secret Service，不写入 SQLite、前端状态快照或日志
...
- 服务器令牌和代理凭据只在 Rust 内存中解密和使用，不通过 Tauri 事件发送给前端
