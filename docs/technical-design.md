...
| 长列表 | 项目内置 `useVirtualRows` | 会话列表按需虚拟化 |
...
| 本地数据库 | SQLite 与 sqlx | 保存服务器、窗口、草稿、待查看结果和偏好 |
...
活动订阅期间先递归脱敏认证字段并将图片与音频 Data URL 替换为类型和数据长度摘要，再写入受条数、总字节和单条大小约束的内存环形缓冲区，并按物理连接与请求 ID 关联请求和响应

追踪批次通过仅允许 `protocol-debug` 窗口访问的 Tauri Channel 投递，调试窗口不具备连接、协议发送、配置或凭据权限

调试窗口销毁、通道投递失败或主动退订时立即停止采集并清空内存，追踪失败不能传播到协议路由或传输生命周期

## 连接管理

### 连接共享

同一应用进程内，相同服务器配置和代理配置版本只维护一条物理连接；单服务器窗口约束由窗口状态仓储独立保证

每个窗口独立保存当前服务器、标签顺序、当前标签和已有会话草稿，同一服务器只允许被一个窗口占用

每个打开的会话标签保持独立的 `thread/resume` 订阅，关闭标签后立即发送 `thread/unsubscribe`

非活动标签保留会话投影但不挂载消息视图，标签切换不保存通用视图快照
...
用户首次展开已完成摘要回合的活动组时，客户端使用 `thread/items/list` 按升序读取一页项目；仍有后续页时在活动组内提供加载更多入口，每次点击只推进一页，最后一页到达后将回合标记为完整视图
...
发送任务创建 turn，运行中输入可以通过 steer 追加或通过 `thread/queue/add` 排到下一回合，显式停止通过中断请求完成
...
连接中断时只在当前进程内保留与服务器和当前 thread 精确匹配的已加载投影，重连期间投影保持只读，冷启动和进程退出后不恢复会话正文
...
| 项目类型 | UI 形态 |
| --- | --- |
| `userMessage` | 安全 Markdown 用户消息、结构化引用和附件摘要 |
| `hookPrompt` | 低强调提示行 |
...
| --- | --- |
| 纯文本和源码 | 内置只读文本查看器 |
| Markdown | 安全渲染与原始源码双视图 |
| HTML | 原始源码视图与系统浏览器打开 |
| JSON | Worker 格式化预览与原始源码双视图 |
...
HTML 不在 WebView 中渲染；用户显式打开时，Rust 层将 app-server 返回的原始内容写入应用专属临时目录，再调用系统默认浏览器

浏览器打开的 HTML 最大 256 MiB，不复制或改写远程工作区的相对资源
...
| 窗口状态 | 尺寸、位置、当前服务器、标签顺序和当前标签 |
| 草稿 | 未发送文本和结构化令牌引用 |
| 待查看结果 | 服务器、会话和最新完成回合标识 |
| 常用提示词 | 全局共享的名称、正文、顺序和并发版本 |
| 最近项目 | 快速创建会话和排序 |
| UI 偏好 | 主题、侧栏宽度和代码折行 |

已有会话草稿在停止输入 500ms 后写入，成功发送后清理对应草稿

空白任务输入只保存在当前进程内，不写入数据库

待查看结果只记录连接期间收到的完成通知，查看时按完成回合标识精确清除，避免较旧的查看操作覆盖较新的完成结果
...
- Markdown、HTML、SVG、JSON 和命令输出均视为不可信内容
- 默认日志不记录用户消息、文件正文、令牌、Cookie、认证头、SSH 认证数据和完整敏感环境变量
- 保存到本机只能使用用户明确选择的目标路径

## 性能策略

- 会话列表使用虚拟化，消息流一次挂载全部回合并使用浏览器原生文档流布局
- 收起的活动组和活动项在尺寸动画完成后卸载详情子树，展开前才挂载 Markdown、JSON 和命令输出详情
- 协议通知在状态层批量归并，每帧最多提交一次纯文本增量渲染
- 代码高亮、全文搜索和大型 JSON 格式化移入 Worker
- 图片按显示尺寸解码，离开预览后释放 Blob URL
- 原始大型命令输出在状态层分块保存，DOM 只保留可视窗口

会话恢复性能诊断在前端内存中保留最近 20 条记录，按请求记录响应等待、JSON 解析、协议校验、投影转换、首次 React 提交、响应字符数、回合数和项目数

性能记录不持久化，不包含服务器地址、thread ID、消息正文或文件正文，通过设置的诊断页查看并随脱敏诊断报告复制
