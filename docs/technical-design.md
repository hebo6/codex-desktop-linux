...
HTML iframe 只允许加载应用创建的 Blob URL，文档自身 CSP 禁止脚本、表单、导航、子框架、对象、网络和 Worker

HTML 的样式元素和行内样式在加载前重写为 Blob 样式表，主 WebView 生产 CSP 不开放 `unsafe-inline`

HTML 相对静态资源通过 app-server 文件接口读取，只接受工作区内普通且非符号链接的 CSS、图片和字体，并限制为最多 64 个、单个 8 MiB、合计 32 MiB
