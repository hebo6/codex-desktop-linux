...
class HandshakeIpc implements HttpProxyWebSocketIpc {
        ...
        this.emit({
          ...
          json: JSON.stringify({
            ...
            result: {
              ...
              platformFamily: "unix",
              platformOs: "linux",
              userAgent: "codex-http-proxy-test",
            },
          }),
        });
      }
...
describe("createHttpProxyWebSocketAppServerSession", () => {
  it("经 HTTP CONNECT 传输完成 app-server 初始化", async () => {
    const ipc = new HandshakeIpc();
    const phases: string[] = [];
    const session = createHttpProxyWebSocketAppServerSession({
      request: {
        connectionId: "remote",
        target: {
